...
def categorize_dish(dish_name, dish_ingredients):
    ...
    Category_strings = ["VEGAN", "VEGETARIAN", "KETO", "PALEO", "OMNIVORE"]

    for i, c in enumerate(Categories):
        if dish_ingredients.issubset(c):
            return dish_name + ": " + Category_strings[i]
...
def tag_special_ingredients(dish):
    ...
    dish_name, allergens = dish

    if isinstance(allergens, list):
        allergens = set(allergens)

    return dish_name, SPECIAL_INGREDIENTS.intersection(allergens)
...
def separate_appetizers(dishes, appetizers):
    ...
    return list(unique_dishes.difference(unique_appetizers))
...
def singleton_ingredients(dishes, intersection):
    ...
    unfiltered_ingredients = set()
    for dish in dishes:
        unfiltered_ingredients.update(dish)
        
    return intersection.symmetric_difference(unfiltered_ingredients)