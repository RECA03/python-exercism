...
def fix_list_of_wagons(each_wagons_id, missing_wagons):
    ...
    """
    new_wag1, new_wag2, wag_id_1, *other_wagons = each_wagons_id
    updated_waglist =[wag_id_1, *missing_wagons, *other_wagons, new_wag1, new_wag2]

    return updated_waglist


def add_missing_stops(route, **single_stops):
    ...
    complete_stops = list()
    for stop in single_stops.items():
        stop_num, stop_name = stop
        complete_stops.append(stop_name)

    route["stops"] = complete_stops
...
def fix_wagon_depot(wagons_rows):
    ...
    fixed_wagons_rows = [list(row) for row in zip(*wagons_rows)]
